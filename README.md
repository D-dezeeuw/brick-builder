...
**Live demo:** https://velvety-blini-d45be3.netlify.app
...
- **Instanced rendering.** One `InstancedMesh` per (shape, color) pair, matrices packed into a single attribute. 10k bricks typically render in ~50 draw calls.
- **Plastic shader.** `MeshPhysicalMaterial` with clearcoat, sheen, and an Oren-Nayar diffuse patch injected via `onBeforeCompile` at medium+. Studio HDRI + ACES Filmic tone mapping. 4 quality presets (Low / Medium / High / Ultra) and independent toggles for AO (N8AO), Bloom, and SMAA.
- **GPU path-traced preview.** Ultra-clean renders via `@react-three/gpu-pathtracer`, 64-sample cap, InstancedMesh expansion at mount because the pathtracer version compatible with three 0.171 predates instancing support.
- **Persistence.** Versioned schema with runtime validation. Autosave to localStorage, sharable lz-string URL hash, JSON + PNG export (render-to-target), JSON import via picker or drag-drop.
- **Multiplayer rooms.** Supabase Postgres + Realtime. Start a room, share the `?r=<id>` URL, live-sync brick placements, title changes, and baseplate growth. No custom server.
- **Mobile-first input.** Tap-and-drag to position the ghost, release to commit. Build / Erase mode toggle. Collapsible sidebar.
...
- **Hosting:** Netlify (static web). No custom server.
...
[`supabase/migrations/001_rooms_and_bricks.sql`](supabase/migrations/001_rooms_and_bricks.sql)
once in the Supabase SQL editor. Without these, solo editing still works —
only room features are disabled.
...
│   │       ├── multiplayer/  # Supabase client + room sync/writes/router
...
- Auth-gated rooms — today anyone with the room id can edit.
...
Netlify auto-deploys from `main`. [`netlify.toml`](netlify.toml) builds only
`@brick/web`, publishes `apps/web/dist`, and includes a SPA fallback for the
`?r=<id>` room routes. The Supabase connection is baked into the client via
`apps/web/.env` — the anon key is safe to commit (RLS gates access).
